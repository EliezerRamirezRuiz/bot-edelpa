...
class MyEvents(Cog):
    ...
    async def on_message(self, message):
        my_handler.emit(makeLogRecord({
            'msg': f"{message.author.name} sent a message: {message.content} ", 
            'levelno': INFO, 
            'levelname':'INFO'}))
    ...
    async def on_command_error(self, ctx, error):
        my_handler.emit(makeLogRecord({
            'msg': f"Ocurrio este error: {error}", 
            'levelno': WARNING, 
            'levelname':'WARNING'}))
    ...
    async def on_command(self, ctx):
        my_handler.emit(makeLogRecord({
            'msg': f"Comando '{ctx.command.name}' fue usado por {ctx.author.name} # {ctx.author.discriminator}",
            'levelno': INFO, 
            'levelname':'INFO'}))
    ...
    async def on_command_completion(self, ctx):
        my_handler.emit(makeLogRecord({
            'msg': f"Comando '{ctx.command}' fue ejecutado exitosamente",
            'levelno': INFO, 
            'levelname':'INFO'}))
    ...
    async def on_member_join(self, member):
        my_handler.emit(makeLogRecord({
            'msg': f"{member} ha entrado al servidor", 
            'levelno': INFO, 
            'levelname':'INFO'}))
    ...
    async def on_member_remove(self, member):
        my_handler.emit(makeLogRecord({
            'msg': f"{member} ha salido del servidor", 
            'levelno': INFO, 
            'levelname':'INFO'}))
...
async def setup(bot:Bot):
    ...
    my_handler.emit(makeLogRecord({
        'msg': "He sido cargado eventos.py", 
        'levelno': INFO, 
        'levelname':'INFO'}))
...
async def teardown(bot:Bot):
    ...
    my_handler.emit(makeLogRecord({
        'msg': "Ha sido descargado eventos.py!", 
        'levelno': INFO, 
        'levelname':'INFO'}))
